...
parser.add_argument('--modeltype', default='MLP_AFD', type=str, help='model type')
...
class Attacker(object):
    ...
    def eval_object(self, eval_funccall, greedy_set, orig_label, greedy_set_visit_idx, query_num,
        ...
        for index in range(n_batches):  # n_batches
            ...
            flag = 0
            # we use the gradient for each false label
        ...
        if Dataset_type[Dataset] == 'multi':
            ...
            max_category = None
        # if the max object changes, we update it and the best funccall
...
log_attack = open(
    './Logs/%s/%s/gradmax_Attack_%s.bak' % (Dataset, Model_Type, args.t), 'w+')
...
if t:
    write_file(Dataset, Model_Type, budget, 'OMPGS', 'gradmax_', time_limit)
