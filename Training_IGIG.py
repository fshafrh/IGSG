...
parser.add_argument('--adv', default='False', type=str, help='adversarial training or not')
...
def calculate_cost(model, X, y, batch_size, attributions, IG_matrix_all):
...
def Training(Dataset, batch_size, n_epoch, lr):
    ...
    IG_matrix_val = torch.tensor([1 / num_feature[Dataset]] * num_feature[Dataset])
    ...
    for epoch in range(n_epoch):
        ...
        if epoch % 10 == 0:
            attributions, IG_matrix_all = IntegratedGradient(X_Train, y_Train, Dataset, n_batches_IG, invalid_sample, model)
        ...
        if epoch % 10 == 0:
            attributions_val, IG_matrix_val = IntegratedGradient(X_Validation, y_Validation, Dataset, n_batches_IG_val,
                                                     invalid_sample, model)
        validate_cost = calculate_cost(model, X_Validation, y_Validation, batch_size, attributions_val, IG_matrix_val)
