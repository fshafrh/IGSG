...
def calculate_cost(model, X, y, batch_size, attributions, valid_mat=None):
...
def Training(Dataset, batch_size, n_epoch, lr):
    ...
    for epoch in range(n_epoch):
        ...
        if epoch % 10 == 0:
            attributions_val, _ = IntegratedGradient(X_Validation, y_Validation, Dataset, n_batches_IG_val,
                                                     invalid_sample, model)
        validate_cost = calculate_cost(model, X_Validation, y_Validation, batch_size, attributions_val, valid_mat)
    ...
    print('alpha, beta:', beta, alpha, file=log_a, flush=True)
